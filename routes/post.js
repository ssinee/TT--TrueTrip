...
router.post('/mypage/write', upload.single('editImg'), function (req, res) {
...
// 프로필 수정할때마다 upload_profile에 하나씩 넣음.
    ...
    Planner.findOneAndUpdate({id: req.user.id}, {
        $set: {
            "location": paramlocation, "introduce": paramintroduce, "orgFileName": paramorgFileName,
            "saveFileName": paramsaveFileName, "path": parampath
        }
    }, function (err, post) {
        if (err) {
            console.log(err);
...
router.post('/mypage/editimg', upload.single('editImg'), function (req, res) {
    if (req.file) {
        fileObj = req.file
        paramorgFileName = fileObj.originalname;// 원본 파일명 저장(originalname은 fileObj의 속성)
        paramsaveFileName = fileObj.filename;//저장된 파일명
